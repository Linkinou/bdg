...
(function ( $ ) {
    ...
    $.fn.readMore = function( options ) {
        ...
        return this.each(function() {
            // Get elements
            var $container = $(this);
            var $expand = $(settings.expandTrigger);
            var $fadeBg = $container.find(settings.fadeBg);

            // Get the height of the container
            var totalHeight = $container[0].scrollHeight;

            // Check if total height is too big
            if(totalHeight <= ($container.height() * 1.5)) {
                ...
                $fadeBg.css('padding-top', (settings.previewHeight/2) + 'px');
            }
            ...
            $expand.on('click', function() {
              ...
              $fadeBg.fadeOut(); // fade out prompt
